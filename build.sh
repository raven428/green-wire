...
r="https://${WRT_OPKG_REPO}/releases/24.10.0"
/usr/bin/env cat <<EOF >prepare/etc/opkg/distfeeds.conf
src/gz openwrt_kmods     ${r}/targets/mediatek/filogic/kmods/6.6.73-1-d649d775435da5a8637f7a955a80d331
...
/usr/bin/env podman run --rm -i --network=host --name=openwrt-builder \
  ...
  ghcr.io/raven428/container-images/owrt-mtk-filogic-24_10_0:000 \
  /usr/bin/env bash -c " \
...
[[ "${WRT_SKIP_INSTALLER:-"none"}" == 'none' ]] && {
  ...
  /usr/bin/env cp -v files/patches/init-d-uhttpd.diff prepare/patches
  /usr/bin/env sed -i \
    "s/option dest_port '80 443'/option dest_port '80 443 5022 5080'/" \
