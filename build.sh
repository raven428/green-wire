...
for conf_file in green-wise/OpenWRT/*; do
...
" "${r}" "${r}" "${r}" "${r}" "${r}" "${r}"
  ...
  /usr/bin/env docker run --user 0 --rm -i --network=host \
    ...
    coreutils-csplit coreutils-cut coreutils-date coreutils-dd coreutils-df \
    coreutils-dir \
    ...
    collectd-mod-netlink collectd-mod-processes collectd-mod-tcpconns \
