...
/usr/bin/env docker run --user 0 --rm -i \
  ...
  make image PROFILE='\''bananapi_bpi-r3'\'' FILES=/files ROOTFS_PARTSIZE=1111 EXTRA_PARTSIZE=6111 PACKAGES='\''lsblk mmc-utils ca-certificates coreutils curl ethtool iperf iperf3 tar nand-utils python3 pv tmux tcpdump terminfo fdisk sfdisk cfdisk gdisk cgdisk xzdiff parted openssh-sftp-server luci bash vim-fuller libgcc libustream-mbedtls nftables losetup resize2fs -dnsmasq dnsmasq-full less shadow-chsh coreutils-mktemp git fping ddns-scripts-cloudflare fail2ban bird2 bird2c bird2cl luci-proto-wireguard diffutils wireguard-tools wg-installer-client atop btop ctop htop iftop ss jq yq rsync hping3 luci-mod-dashboard luci-app-ddns mutt blkid strace coreutils-dd dockerd docker-compose prometheus-node-exporter-lua shadow-usermod	prometheus-node-exporter-lua-nat_traffic prometheus-node-exporter-lua-netstat prometheus-node-exporter-lua-openwrt docker prometheus-node-exporter-lua-thermal prometheus-node-exporter-lua-wifi prometheus-node-exporter-lua-wifi_stations grep xzgrep block-mount zram-swap file procps-ng-free procps-ng-pgrep procps-ng-pkill procps-ng-pmap procps-ng-pwdx bspatch procps-ng-skill procps-ng-slabtop lm-sensors logger patch procps-ng-snice procps-ng-sysctl procps-ng-vmstat procps-ng-w  procps-ng-watch procps-ng-top procps-ng-snice procps-ng-kill procps-ng-ps  procps-ng-uptime monit coreutils-cp coreutils-rm coreutils-mv coreutils-sort bsdiff coreutils-rmdir coreutils-ln xzless ack grep xzgrep zoneinfo-all openssl-util msmtp bind-dig bind-host iputils-ping iputils-arping iputils-clockdiff iputils-tracepath mtr-json acme-acmesh-dnsapi luci-app-acme iputils-clockdiff nfdump softflowd flock msmtp-queue shadow-passwd shadow-chpasswd podman coreutils-chown coreutils-chmod'\'''
