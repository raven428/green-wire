...
function M.response.send(self, applet)
  ...
  if not self.headers["content-type"] then
    if type(self.content) == "table" then
      applet:add_header("content-type", "application/json; charset=" .. self.encoding)
      ...
      applet:add_header("content-type", "text/plain; charset=" .. self.encoding)
    end
  end
...
function M.request.parse(applet)
  ...
  if (applet.method == "POST" or applet.method == "PUT") and applet.length > 0 then
...
function M.send(method, t)
  if type(t) ~= "table" then
    return nil, "http." .. method:lower() .. ": exp Request object for named parameters"
  end
  ...
  if c then
    ...
    while true do
      ...
      if not r.status_code then
        ...
        if not _ then
          socket:close()
          return nil, "http." .. method:lower() .. ": Could not parse request line"
        end
