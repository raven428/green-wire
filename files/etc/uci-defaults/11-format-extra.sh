...
# nodiscard should be removed in case of non-sdcard
/usr/bin/env uci set "fstab.${uci_record}.options"='noatime,nodiscard,alloc_mode=reuse'
uci_record="$(/usr/bin/env uci add fstab mount)"
...
/usr/bin/env uci set "fstab.${uci_record}.enable"='1'
/usr/bin/env uci set "fstab.${uci_record}.options"='noatime'
